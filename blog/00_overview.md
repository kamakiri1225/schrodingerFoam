...
位相図（右）がチカチカして見えるのは、背景全体が大域位相 $e^{-i\mu t}$ で一様に回っているため。これは物理的に無意味な位相なので、$H$ の代わりに $H-\mu$ を解く（＝回転系へ移る）と背景の色が止まり、**渦の位相構造だけがくっきり残ります**。密度や渦の運動には一切影響しません。詳しい理由と実装は第6回で解説しています。

<p align="center">
  <img src="../figures/03_1_vortexDipole_muShift/density_phase_muShift.gif" width="680"><br>
  <em>$H-\mu$ を解いた版：背景の位相が静止し、渦の構造だけがくっきり見える（密度・渦の運動は上と同一）</em>
</p>
...
<p align="center">
  <img src="../figures/04_1_darkSoliton_whiteNoise_muShift/density_phase_muShift.gif" width="720"><br>
  <em>密度（左）と位相（右）。まっすぐな2本の溝が波打って密度ゼロの渦核へ分裂していく（背景の位相を静止させた版）</em>
</p>

密度図（左）では2本の溝が横方向にうねって節にちぎれ、位相図（右）では分裂した点のまわりで位相が $2\pi$ 巻く（渦-反渦対の芽）様子が見えます。位相の背景は、大域位相 $e^{-i\mu t}$ の回転を取り除く（$H-\mu$ を解く＝回転系へ移る）ことで静止させてあり、各渦の位相構造だけがくっきり残ります。計算の間ずっとノルムは一定で、第1回で述べた前進オイラー版の発散を、Crank–Nicolson できちんと克服できていることも確認できます。
...
ケースの設定ファイルとコードは、リポジトリの `run/01_darkSoliton_realTime`（決定論的な種）／`run/04_darkSoliton_noSeed`・`run/04_darkSoliton_whiteNoise`（種なし・白色ノイズの対照）／`run/04_1_darkSoliton_whiteNoise_muShift`（μ差し引き版）にあります。
...
3. **易しい検証から難しい本番へ。** トンネル効果・調和振動子・自由膨張（第2〜4回）で「ちゃんと解けている」ことを確かめ、虚時間発展（第5回）で初期状態を作り、量子渦（第6回）とダークソリトン崩壊（本総集編）という非線形の本番に到達した。
...
- **総集編（第0回）：なぜ流体ソルバでシュレーディンガー方程式が解けるのか（本記事）** — シリーズ全体のダイジェスト。ダークソリトン崩壊まで一望できます。

本編（各回の詳細）：

1. [【OpenFOAMで量子力学（１）】シュレーディンガー方程式ソルバを自作する｜laplacianFoam改造4ステップ](01_solver.md)
2. [【OpenFOAMで量子力学（２）】量子トンネル効果を1次元シミュレーション｜透過率を理論と比較](02_tunneling.md)
3. [【OpenFOAMで量子力学（３）】調和振動子のコヒーレント状態を1次元シミュレーション](03_harmonicOscillator.md)
4. [【OpenFOAMで量子力学（４）】波束の広がりは量子と古典でどう違うか｜2次元自由膨張](04_quantumVsClassical.md)
5. [【OpenFOAMで量子力学（５）】虚時間発展で基底状態を作る仕組みを解説](05_imaginaryTime.md)
6. [【OpenFOAMで量子力学（６）】走る渦対（渦-反渦ダイポール）をシミュレーション](06_vortexDipole.md)
