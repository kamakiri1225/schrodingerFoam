...
なお、この崩壊も位相図で見ると背景全体が大域位相 $e^{-i\mu t}$ でチカチカします。第6回と同じく $H-\mu$ を解く（回転系へ移る）と背景が止まり、**崩壊してできた1つ1つの渦の位相構造だけ**がくっきり残ります。下は白色ノイズ崩壊を $\mu$ を引いて解いた版で、密度（左）で渦への崩壊が、位相（右）で各渦の $2\pi$ 巻きが、背景の点滅なしに見えます（`run/04_1_darkSoliton_whiteNoise_muShift`）。

<p align="center">
  <img src="../figures/04_1_darkSoliton_whiteNoise_muShift/density_phase_muShift.gif" width="720"><br>
  <em>白色ノイズ崩壊を $H-\mu$ で解いた版：密度（左）と位相（右）。背景がチカチカせず、崩壊後の各渦の位相構造が見やすい</em>
</p>
...
7. **【OpenFOAMで量子力学（７）】2次元ダークソリトンの崩壊｜ノイズの有無で運命が変わる（本記事）**
