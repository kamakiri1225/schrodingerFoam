...
| **M2** | He＋Hartree（Poisson結合） | 🔨 実装済み・検証中（§7） |
| **M3** | LDA交換＋SCF完成 | コードは配線済み（`xc slater`）、未検証 |
...
## 7. M2 の設計（He＋Hartree、進行中）

- `occupation 2`（1s²）、`Z=2`、`hartree true`。
- $V_H$ の境界条件：遠方で $V_H\to N_e/r$（単極子）なので、箱の壁に Dirichlet で $2/r$ を与える（`0/VH` の `#codeStream`）。
- **参照値**（径方向Hartree-SCFをPythonで独立計算、同モデル）：
  - a=0.4：$\varepsilon=-0.0842$, $E_H=0.9732$, $E_{\rm tot}=2\varepsilon-E_H=\mathbf{-1.1418}$ Ha
  - （検算：He⁺ a=0.4 は −1.2086、a→0 で −2.0 ✓）
- **物理メモ＝自己相互作用誤差（SIE）**：この「$V_H$ を全密度から作る」Hartree近似は、電子が**自分自身の作る反発まで感じてしまう**。純Coulomb極限で $E\approx-1.95$ Ha となり、Hartree–Fock の $-2.86$ から大きく外れるのはこのため（2電子系では交換がちょうど $E_H$ の半分を打ち消す）。**M3のLDA交換がこれを近似的に補正する**——DFTで $V_{xc}$ が本質的な理由が、実装するとエネルギーの数値でそのまま見える。
