...
### 鍵となる構造：純オフダイアゴナル連成（なぜ realTime は `fvm::` にできないか）

実時間の方程式を (u,v) のベクトルでまとめると

$$\frac{\partial}{\partial t}\begin{pmatrix}u\\ v\end{pmatrix}
  =\begin{pmatrix}0 & +H\\ -H & 0\end{pmatrix}\begin{pmatrix}u\\ v\end{pmatrix},
  \qquad H=-D\nabla^2+W.$$

- **連成**：$u$ の増え方は $v$ で、$v$ の増え方は $u$ で決まる（切り離せない）。
- **オフダイアゴナル**：対角成分（$u\to u,\ v\to v$）が **0**、結合は非対角 $\pm H$ にある。
- **純（purely）**：$u$ の式に $u$ 自身が出てこない（$v$ だけ）。ここが肝。

`fvm::`（陰的）は「**未知数が自分の式に現れる**」ときにその項を行列の**対角**に組んで解く仕組み。
拡散 $\partial_t T=\nabla^2 T$ は右辺に $T$ が居る＝対角に自分がいるので陰的化できる。
ところが実時間の $u$ の式の右辺は $Hv$ で $u$ が居ない → **対角に組む自分の項が無い**
→ 単一場の `fvm::` では陰的にできない。off-diagonal（別場との結合）を陰的に扱うには
**2×2ブロック行列**が要る（＝ 6.17(E)／6.16 の PETSc・ブロック行列）。
それが無いので realTime は `fvc::`（明示評価）＋Picard反復で書いている
（`fvm::` が効くのは、各場が自分自身の拡散でダンプされる虚時間側だけ）。

**物理的意味**：$\begin{pmatrix}0&H\\-H&0\end{pmatrix}$ は**反対称**＝ (u,v) 平面の**回転**そのもの。
だから長さ $|\psi|^2=u^2+v^2$ が保たれる（＝ノルム保存＝ユニタリ）。
対角に負の値なら**減衰**（拡散・虚時間発展）、非対角の反対称なら**回転**（実時間発展）。
